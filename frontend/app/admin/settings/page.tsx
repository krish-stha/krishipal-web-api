...
import { adminGetSettings, adminUpdateSettings } from "@/lib/api/admin/settings";
...
export default function AdminSettingsPage() {
  ...
  const save = async () => {
    ...
    const enabledCount =
        Number(payload.payments.COD) + Number(payload.payments.KHALTI) + Number(payload.payments.ESEWA);
    ...
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-5xl p-4 sm:p-6">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-600 mt-1">Store profile, shipping, inventory, payments.</p>
        </div>
        ...
        <div className="grid gap-6">
          ...
          <Card className="rounded-3xl border-slate-200 bg-white shadow-sm p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="block">
                <div className="text-xs font-medium text-slate-600 mb-1">Store Name</div>
                <input
                  value={form.storeName || ""}
                  onChange={(e) => setForm((p: any) => ({ ...p, storeName: e.target.value }))}
              ...
              <label className="block">
                <div className="text-xs font-medium text-slate-600 mb-1">Store Email</div>
                <input  type="email"
                  value={form.storeEmail || ""}
                  onChange={(e) => setForm((p: any) => ({ ...p, storeEmail: e.target.value }))}
                  className="w-full h-11 rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-300"
                />
              </label>

              <label className="block sm:col-span-2">
                <div className="text-xs font-medium text-slate-600 mb-1">Store Address (invoice footer)</div>
                <textarea
                  value={form.storeAddress || ""}
                  onChange={(e) => setForm((p: any) => ({ ...p, storeAddress: e.target.value }))}
          ...
          <Card className="rounded-3xl border-slate-200 bg-white shadow-sm p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="block">
                <div className="text-xs font-medium text-slate-600 mb-1">Default Shipping Fee (Rs)</div>
                <input
                  type="number" min={0}
                  value={form.shippingFeeDefault ?? 0}
                  onChange={(e) => setForm((p: any) => ({ ...p, shippingFeeDefault: e.target.value }))}
                  className="w-full h-11 rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-300"
                />
              </label>

              <label className="block">
                <div className="text-xs font-medium text-slate-600 mb-1">Free Shipping Threshold</div>
                <input
                  type="number"
                  placeholder="Leave empty to disable"
                  value={form.freeShippingThreshold ?? ""}
                  onChange={(e) => setForm((p: any) => ({ ...p, freeShippingThreshold: e.target.value }))}
                  className="w-full h-11 rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-300"
                />
              </label>
            </div>
        ...
        <div className="mt-3 grid gap-2 text-sm">
            ...
            <span className="font-semibold text-slate-900">Rs. {num(form.shippingFeeDefault, 0)}</span>
            </div>

            <div className="flex justify-between">
            <span className="text-slate-600">Free shipping over</span>
            <span className="font-semibold text-slate-900">
                {form.freeShippingThreshold === null || form.freeShippingThreshold === ""
                ? "Disabled"
            ...
            <span className="font-semibold text-slate-900">{Math.max(1, num(form.lowStockThreshold, 5))}</span>
            </div>

            <div className="mt-2 rounded-2xl border border-slate-200 bg-slate-50 p-3">
            <div className="text-xs text-slate-500 mb-2">Enabled payments</div>
            <div className="flex flex-wrap gap-2">
                {form.payments?.COD && <span className="px-2 py-1 rounded-full text-xs font-semibold bg-white border">COD</span>}
                {form.payments?.KHALTI && <span className="px-2 py-1 rounded-full text-xs font-semibold bg-white border">KHALTI</span>}
                {form.payments?.ESEWA && <span className="px-2 py-1 rounded-full text-xs font-semibold bg-white border">ESEWA</span>}
                {!form.payments?.COD && !form.payments?.KHALTI && !form.payments?.ESEWA && (
                <span className="text-xs text-rose-600 font-semibold">None enabled (not allowed)</span>
                )}
            </div>
            </div>
        </div>
          ...
          <Card className="rounded-3xl border-slate-200 bg-white shadow-sm p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="block">
                <div className="text-xs font-medium text-slate-600 mb-1">Low Stock Threshold</div>
                <input
                  type="number"  min={1}
                  value={form.lowStockThreshold ?? 5}
                  onChange={(e) => setForm((p: any) => ({ ...p, lowStockThreshold: e.target.value }))}
                  className="w-full h-11 rounded-2xl border border-slate-200 px-3 text-sm outline-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-300"
                />
              </label>
          ...
          <Card className="rounded-3xl border-slate-200 bg-white shadow-sm p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3 text-sm">
              {["COD", "KHALTI", "ESEWA"].map((k) => (
                <label key={k} className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-3">
                  <input
                    type="checkbox"
                    ...
                    onChange={(e) =>
                      setForm((p: any) => ({
                        ...p,
                        payments: { ...(p.payments || {}), [k]: e.target.checked },
                      }))
                    }
                  />
            ...
            <div className="mt-3 text-xs text-slate-500">
              Disabling a method should hide it in checkout (frontend can read these settings).
            </div>
          </Card>
